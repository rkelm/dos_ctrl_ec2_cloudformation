...
%AWS_BIN% ec2 create-snapshot --volume-id %VOLUMEID% --description "%1 %APP_NAME% Snapshot created %DATE% %TIME%." --output text --query SnapshotId > snapshotid.txt
...
IF NOT DEFINED _NAMEVALUE (
  %AWS_BIN% ec2 describe-volumes --volume-id %VOLUMEID% --output text --query Volumes[*].Tags[?Key==`Name`].Value > output.txt  
  SET /P _NAMEVALUE=<output.txt
)
IF DEFINED _NAMEVALUE (
  %AWS_BIN% ec2 create-tags --resources %SNAPSHOTID% --tags "Key=Name,Value=%_NAMEVALUE%"
)
%AWS_BIN% ec2 create-tags --resources %SNAPSHOTID% --tags "Key=%TAGKEY%,Value=%TAGVALUE%"

REM Show snapshot size to user.
%AWS_BIN% ec2 describe-volumes --volume-id %VOLUMEID% --output text --query Volumes[*].Size > output.txt  
