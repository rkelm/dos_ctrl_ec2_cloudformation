...
%AWS_BIN% ec2 run-instances --image-id %IMAGEID% --instance-type %INSTANCETYPE% %KEYPAIR_PARAM% %SECURITYGROUPSID_PARAM% --instance-initiated-shutdown-behavior terminate --region %REGION% %SUBNETID_PARAM% %SSM_ROLE_NAME_PARAM% --user-data file://prepare_server.sh --output text --query Instances[*].InstanceId > %INSTIDFILE%
SET INSTANCEID=EMPTY
SET /P INSTANCEID=<%INSTIDFILE%
...
IF NOT [%SNS_TOPIC_ARN%] == [] (
	%AWS_BIN% sns publish --topic-arn "%SNS_TOPIC_ARN%" --subject "STARTE %APP_NAME% Server auf ec2 Instanztyp %INSTANCETYPE%" --message "Starte %APP_NAME% Server auf %INSTANCETYPE% mit Instanz ID %INSTANCEID%. (%DATE% %TIME%)" --output text > messageid.txt
)

REM Tag Instance for easy identification by 
REM other clients without knowledge of instance id.
%AWS_BIN% ec2 create-tags --resources %INSTANCEID% --tags Key=%TAGKEY%,Value=%TAGVALUE%
...
IF DEFINED VOLUMEID (
	...
	IF DEFINED VOLUMETAGKEY (
		...
		%AWS_BIN% ec2 describe-volumes --region %REGION% --filters Name=status,Values=available Name=tag:%VOLUMETAGKEY%,Values=%VOLUMETAGVALUE% --output=text --query Volumes[*].VolumeId > volumeid.txt
		...
		IF DEFINED SNAPSHOTID (
			...
			%AWS_BIN% ec2 describe-snapshots --region %REGION% --filters Name=status,Values=completed Name=tag:%SNAPSHOTTAGKEY%,Values=%SNAPSHOTTAGVALUE% --output=text --query Snapshots[*].SnapshotId > snapshotid.txt
		...
		%AWS_BIN% ec2 describe-subnets --region %REGION% --filter Name=subnet-id,Values=%SUBNETID% --output text --query Subnets[*].AvailabilityZone > availabilityzone.txt
		...
		%AWS_BIN% ec2 create-volume --region %REGION% --availability-zone !_AVAILABILITYZONE! --snapshot-id !_SNAPSHOTID! --volume-type gp2 --tag-specifications "ResourceType=volume,Tags=[{Key=Name,Value=TEMPORARY_%APP_NAME%}]" --query VolumeId --output text > volumeid.txt
		SET /P _VOLUMEID=<volumeid.txt
...
%AWS_BIN% ec2 wait instance-running --instance-ids %INSTANCEID%
%AWS_BIN% ec2 wait instance-running --instance-ids %INSTANCEID%
...
%AWS_BIN% ec2 describe-instances --instance-ids %INSTANCEID% --output text --query Reservations[*].Instances[*].PublicIpAddress > ipaddress.txt
...
%AWS_BIN% ec2 attach-volume --volume-id %_VOLUMEID% --instance-id %INSTANCEID% --device /dev/sdf > attachvolume.json
