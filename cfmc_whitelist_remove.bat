...
ECHO Bitte geben Sie den Minecraft User Namen ein, der aus der Whitelist entfernt werden soll.
SET /P _input=
...
CALL cf_send_cmd.bat %_CONFIG% app_cmd.sh 'whitelist remove %_output%' 'kick %_output%'
REM CALL %CTRL_PATH%ec2_send_command.bat %_CONFIG% sudo -u ec2-user %SRV_INSTALL_PATH%/bin/app_cmd.sh "kick %_output%"
