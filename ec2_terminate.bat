...
%AWS_BIN% ec2 terminate-instances --instance-ids %INSTANCEID% > terminate.json
...
%AWS_BIN% ec2 wait instance-terminated --instance-ids %INSTANCEID%

IF NOT ERRORLEVEL 1 (
	ECHO Die Instanz ist terminiert.
	REM Send notice about terminated instance.
	IF NOT [%SNS_TOPIC_ARN%] == [] (
		%AWS_BIN% sns publish --topic-arn "%SNS_TOPIC_ARN%" --subject "BEENDE %APP_NAME% Server mit Instanz ID %INSTANCEID%" --message "Beende %APP_NAME% Server mit Instanz ID %INSTANCEID%. (%DATE% %TIME%)" --output text > messageid.txt
	)
)
